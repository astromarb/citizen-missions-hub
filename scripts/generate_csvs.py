...
# ─── Locations ──────────────────────────────────────────────────────────────
V3 = os.path.join(ROOT, "supabase/migrations/20260527000002_locations_v3.sql")

with open(V3) as f:
    content = f.read()

# Extract the big INSERT VALUES block (Section 4)
loc_block_m = re.search(
    r"INSERT INTO public\.locations\s*\([^)]+\)\s*VALUES(.*?)ON CONFLICT \(system, body, name\)",
    content, re.DOTALL
)
if not loc_block_m:
    raise RuntimeError("Could not find Section 4 INSERT block in v3 SQL")
...
for m in row_re.finditer(block):
    ...
    if notes_raw == "NULL":
        ...
        notes = notes_raw.strip("'").replace("''", "'")
    locations.append([system, body, name, sort_order, loc_type,
                      is_active, is_trade, is_freight, confidence, notes])

# Add Port Olisar (decommissioned; lives in DB from v1/v2 but not in v3 Section 4)
locations.append([
